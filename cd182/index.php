...
{
    $proxy->clearCookies();
    echo '<meta http-equiv="refresh" content="0; url=' . cdURL . '">';
}
...
{
    function outside_handler($e)
    {
        global $proxy, $error_string, $settings, $errorTemplate;
        $m = $e->getMessage();
        if (!empty($m) && !empty($errorTemplate))
        {
            $error_string = $m;
...
else
{
    ...
    if (!empty($miniFormTemplate))
    { //Get our mini-form file with PHP code compiled for result
        ob_start();
        include ("" . $miniFormTemplate . "");
...
function findTemplate($themeName, $location = "home", $startLocation = "")
{
    if (file_exists(BASE_DIRECTORY . DS . "plugins" . DS . $themeName))
    {
        ...
        {
            $startLocation = BASE_DIRECTORY . "plugins" . DS . (!empty($themeName) ? $themeName . DS : "") . "*";
        }
